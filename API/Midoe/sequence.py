...
from rtmidi.midiconstants import (ALL_SOUND_OFF, BANK_SELECT_LSB, BANK_SELECT_MSB, CHANNEL_VOLUME, CONTROL_CHANGE, NOTE_ON, PROGRAM_CHANGE)
...
def main(args=None):
    ...
    aadd('-b', '--bpm', type=float, default=100, help="Beats per minute (BPM) (default: %(default)s)")
    aadd('-c', '--channel', type=int, default=10, metavar='CH', help="MIDI channel (default: %(default)s)")
    aadd('-p', '--port', help='MIDI output port number (default: ask)')
    aadd('-k', '--kit', type=int, metavar='KIT', help='Drum kit MIDI program number (default: none)')
    aadd('--bank-msb', type=int, metavar='MSB', help='MIDI bank select MSB (CC#00) number (default: none)')
    aadd('--bank-lsb', type=int, metavar='MSB', help='MIDI bank select LSB (CC#32) number (default: none)')
    aadd('-H', '--humanize', type=float, default=0.0, metavar='VAL', help='Random velocity variation (float, default: 0, try ~0.03)')
    aadd('pattern', nargs='?', type=argparse.FileType(), help='Drum pattern file (default: use built-in pattern)')
    ...
    print(f"Playing drum loop at {seq.bpm} BPM, press Control-C to quit.")
