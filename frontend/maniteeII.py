...
def do_it(place_holder=0):
    ...
    def sine_wave(f, detune=0.0):
        ...
        return y
    """
    def square_wave(f, detune=0.0):
        a = np.sin((f + detune) * x + ramp0 *
                np.sin(((f + detune) * 0.5) * x + (
                    np.sin(((f + detune) * fm) * x) * 0.5))) + 0.5
    ...
    for i in range(-5, 10):
        factor = (2**(i / 12.0))
        waveform_mod = square_wave(freq1 * factor)
        waveform = square_wave(freq1 * factor)
        waveform_detune = square_wave(freq1 * factor, freq5)
