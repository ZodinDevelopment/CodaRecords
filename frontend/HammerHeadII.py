    ...
    def triangle(f, detune=0.0):
        #y = 2 / np.pi * np.arcsin(np.sin((f + detune) * x + ramp0 * (
                #2 / np.pi * np.arcsin(np.sin((f + detune) * 0.5) * x + (
                        #2 / np.pi * np.arcsin(np.sin(((f + detune) * fm) * x) * 0.5))))))

        #y = 2 / np.pi * np.arcsin(np.sin((f + detune) * x + ramp0))

        #y = y * 2 / np.pi * np.arcsin(np.sin((f + detune) * 0.5)) * x

        #y = y + (2 / np.pi * np.arcsin(np.sin(((f + detune) * fm) * x) * 0.5))

        #y = y * np.sin(((f + detune) * 0.5) * x + (
...
master.title("Triangle Based Synthboard - HammerHeadII")

for i in keys:
    binders(f'{i}')

master.bind("<ButtonRelease-1>", do_it)
master.bind("<z>", toggle_wave1)
#master.bind("<x>", toggle_wave2)
wave1="sine"
...
scale_f1 = tk.Scale(master, from_=110, to=660, resolution=220, orient=tk.HORIZONTAL, length=50)
